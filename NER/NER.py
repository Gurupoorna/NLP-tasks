...
with open('svmclassifier-10000.pkl','rb') as f:
    svmclassifier = pickle.load(f)
with open(f'red2v-10000.pkl', 'rb') as f:
    red2v = pickle.load(f)
...
if __name__ == '__main__':
    ...
    ttfrac = trainsize/fulltrainsize
    testsize = fulltestsize  #int(ttfrac*fulltestsize)
    ...
    for s in propsts['sents_list']:
        for w in s:
            if i>rr:
                print(f"{w:20s} - {propsts['ys'][i]:6s} - {predictions[i] if predictions[i]==1 else ''}")
            i+=1
            if i-rr>40: break
        if i-rr>40: break
